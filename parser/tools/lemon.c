...
struct lemon {
  ...
  char *outsrcname;        /* Name of the output source file */
  char *outhdrname;        /* Name of the output header file */
...
static char *outputsrc_name = NULL;
static void handle_o_option(char *z){
  outputsrc_name = (char *) malloc( lemonStrlen(z)+1 );
  if( outputsrc_name == NULL ){
    memory_error();
  }
  lemon_strcpy(outputsrc_name, z);
}

static char *outputhdr_name = NULL;
static void handle_d_option(char *z){
  outputhdr_name = (char *) malloc( lemonStrlen(z)+1 );
  if( outputhdr_name == NULL ){
    memory_error();
  }
  lemon_strcpy(outputhdr_name, z);
}
...
int main(int argc, char **argv)
{
  ...
  static struct s_options options[] = {
    ...
    {OPT_FSTR, "d", (char*)handle_d_option, "Specify the output header file."},
  ...
  if (outputsrc_name != NULL)
      lem.outsrcname = outputsrc_name;
  else
      lem.outsrcname = file_makename(&lem, ".c");
  
  if (outputhdr_name != NULL)
      lem.outhdrname = outputhdr_name;
  else
      lem.outhdrname = file_makename(&lem, ".h");
...
PRIVATE void tplt_print(FILE *out, struct lemon *lemp, char *str, int *lineno)
{
  ...
  if (!lemp->nolinenosflag) {
    (*lineno)++; tplt_linedir(out,*lineno,lemp->outsrcname); 
  }
  return;
}
...
PRIVATE void emit_code(
 ...
 if( rp->code ){
   ...
   if( !lemp->nolinenosflag ){
     (*lineno)++;
     tplt_linedir(out,*lineno,lemp->outsrcname);
   }
 }
